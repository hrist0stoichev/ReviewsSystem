...
func NewEmailsService(host, port, from, username, password, subject, genericMessage, confirmationURL, queryParameter string, tokenLength int, randGenerator *rand.Rand) EmailsService {
...
func (es *emailsService) SendConfirmationEmail(to, token string) error {
	msg := fmt.Sprintf(msgFormat, es.from, to, es.subject, es.msg, es.confirmationURL, es.queryParameter, token)
